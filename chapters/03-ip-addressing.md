...
在企业网络中，终端地址不是孤立配置出来的，而是从规划、分配、使用、回收形成一个生命周期。理解这个过程，有助于排查 DHCP、地址冲突和资产不一致的问题。

典型流程如下：

```mermaid
flowchart LR
    A[地址规划] --> B[创建 VLAN 和网关]
    B --> C[配置 DHCP 地址池]
    C --> D[终端接入并获取地址]
    D --> E[交换机学习 MAC]
    E --> F[网关学习 ARP]
...
| 阶段 | 正常结果 | 常见问题 |
| --- | --- | --- |
| 地址规划 | VLAN、网段、网关、DNS 已明确 | 网段重叠，网关不统一 |
| DHCP 地址池 | 地址范围、掩码、网关、DNS 正确 | 地址池包含保留地址，DNS 下发错误 |
| 终端接入 | 终端拿到所属 VLAN 的地址 | 接入口 VLAN 错误，拿到其他网段地址 |
| MAC 学习 | 交换机在正确 VLAN 学到终端 MAC | MAC 漂移，环路，端口接错 |
| ARP 学习 | 网关能解析终端 IP 到 MAC | 地址冲突，ARP 表频繁变化 |
| 地址释放 | 租约到期后地址可复用 | 长租约导致地址池耗尽 |
...
如果某台办公电脑拿到 `10.10.50.80/24`，不要只改电脑 IP。应该检查它接入的端口是否被放进了访客 VLAN 50，或者无线控制器是否把员工 SSID 映射错了 VLAN。

## 3.17 地址类故障排查表

地址问题经常表现为“有时通、有时不通”或“同部门别人正常，只有我异常”。排查时建议先确认终端四件套：IP、掩码、网关、DNS。

| 现象 | 优先怀疑 | 验证方法 | 下一步 |
| --- | --- | --- | --- |
| IP 为 `169.254.x.x` | DHCP 未成功 | 查看 DHCP 租约、接入口 VLAN | 检查 DHCP Relay 和地址池 |
| IP 属于错误网段 | VLAN 或 SSID 映射错误 | 对比端口 VLAN 和 DHCP 地址池 | 修正接入口或无线策略 |
| 能 ping 同网段，不能上网 | 网关、路由、NAT、策略 | ping 网关、traceroute、公网 IP | 查默认路由和防火墙 |
| 能 ping IP，不能访问域名 | DNS 错误 | `nslookup` 指定 DNS 查询 | 查 DNS 下发和服务器状态 |
| 间歇性断线 | 地址冲突或 ARP 异常 | 查看 ARP 表、MAC 地址表、日志 | 定位重复 IP 设备 |
| 只能访问部分同网段地址 | 掩码错误 | 对比终端掩码和规划 | 修正 DHCP 或手工地址 |

地址排错时要避免一个误区：看到终端 IP 不对，就直接手工配置一个正确 IP。这样可能让单台电脑临时恢复，但会掩盖 DHCP、VLAN 或地址池问题。企业网络中应该优先修正集中配置，让同类终端自动获得一致的地址参数。
