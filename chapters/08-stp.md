...
STP 属于“平时很安静，出问题影响很大”的技术。企业网络中不要依赖默认选举结果，而要在设计阶段就明确根桥、备根桥、阻塞链路和保护机制。

设计流程可以按下面顺序：

| 步骤 | 需要确定的内容 | 示例 |
| --- | --- | --- |
| 1 | 哪些 VLAN 需要二层跨设备 | 办公 VLAN 10 跨接入到核心 |
| 2 | 根桥放在哪里 | Core-1 为 VLAN 10 主根 |
| 3 | 备根桥放在哪里 | Core-2 为 VLAN 10 备根 |
| 4 | 哪些端口是接入口 | PC、打印机、摄像头端口 |
| 5 | 哪些端口是互联口 | 接入到核心、核心互联 |
| 6 | 需要哪些保护 | 边缘端口、BPDU 保护、根保护 |
| 7 | 如何验证切换 | 断开主上联，观察收敛和业务 |

变更 STP 时要特别谨慎，因为根桥变化可能导致大量 VLAN 的转发路径变化。建议维护窗口内按下面方式操作：

```text
变更前：
查看当前根桥、端口角色、阻塞端口、拓扑变化次数。

变更中：
先调整备根，再调整主根；一次只改一个范围；观察拓扑是否稳定。

变更后：
确认根桥符合设计，业务 VLAN 路径符合预期，日志中没有持续拓扑变化。
```

如果需要在生产中新增接入交换机，应先确认它的 STP 优先级不会低于核心设备。否则一台新接入设备可能因为桥 ID 更小而成为根桥，导致业务流量绕路甚至不稳定。

## 8.15 拓扑变化分析

STP 网络中偶发卡顿、语音中断或终端短暂掉线，可能与拓扑变化有关。拓扑变化不是一定有问题，例如链路切换时会发生正常变化；但如果拓扑变化频繁，就要排查二层不稳定。

常见原因：

| 现象 | 可能原因 | 验证方向 |
| --- | --- | --- |
| 拓扑变化计数持续增加 | 链路抖动或端口反复 up/down | 查看接口日志和光功率 |
| MAC 地址频繁漂移 | 二层环路或聚合异常 | 查 MAC flapping 日志和聚合状态 |
| 接入口触发 BPDU 保护 | 用户私接交换机或端口类型错误 | 查端口连接对象 |
| 根桥突然变化 | 新设备优先级更低或配置错误 | 查看桥 ID 和根桥历史 |
| 某 VLAN 不通，其他 VLAN 正常 | MSTP 实例或 VLAN 映射异常 | 查实例和 VLAN 绑定 |

排查 STP 相关问题时，要把“时间线”记录下来：

```text
09:10 用户反馈网络卡顿
09:11 Access-1 GE0/0/24 down/up
09:11 STP topology change
09:12 Core 学到 MAC 地址漂移
09:13 业务恢复
```

有了时间线，才能判断是链路故障引发 STP 收敛，还是 STP 配置错误引发链路和 MAC 异常。生产排错中，日志时间、接口状态、STP 状态和用户故障时间必须对齐看。

## 8.16 STP 实验建议

STP 很适合通过实验理解。只看文字容易记住“根桥、根端口、阻塞端口”，但不知道端口为什么这样选，也不知道链路断开后会发生什么。
...
| 步骤 | 操作 | 观察点 |
| --- | --- | --- |
| 1 | 三台交换机默认配置互联 | 谁自动成为根桥 |
| 2 | 手动降低 SW1 优先级 | 根桥是否变为 SW1 |
| 3 | 设置 SW2 为备根 | SW1 故障后 SW2 是否接管 |
| 4 | 查看 SW3 端口角色 | 哪个是根端口，哪个被阻塞 |
| 5 | 断开 SW3 到 SW1 的链路 | 阻塞端口是否转发 |
| 6 | 在 PC 接入口启用边缘端口 | 终端接入是否更快转发 |
| 7 | 在 PC 口接入小交换机 | BPDU 保护是否触发 |

每一步都要记录拓扑变化前后的端口角色。不要只写“业务恢复”，还要写清楚哪个端口从阻塞变为转发、收敛大约用了多久、日志中是否出现拓扑变化。

## 8.17 STP 与其他技术的关系

STP 不应该孤立理解。它经常和 VLAN、链路聚合、三层网关、无线 AP 接入一起出现。

| 技术 | 与 STP 的关系 |
| --- | --- |
| VLAN | STP 通常按 VLAN 或实例计算二层拓扑 |
| Trunk | Trunk 承载多个 VLAN，阻塞会影响多个业务 |
| 链路聚合 | 聚合后 STP 应看到一个逻辑端口 |
| 三层网关 | 三层边界可以缩小二层环路范围 |
| VRRP | 根桥和网关主设备最好路径一致 |
| AP 接入 | AP 上联可能是 Trunk，但不应形成二层环路 |

例如 Core-1 是 VLAN 10 的 VRRP 主网关，但 STP 根桥却在 Core-2，流量可能先走到 Core-2 再回到 Core-1，形成不必要绕路。设计时常见做法是让主要业务 VLAN 的 STP 根桥与网关主设备保持一致，备核心作为备根和备网关。

这不是硬性规则，但它体现了一个设计原则：二层转发路径、三层网关位置和冗余主备关系应该互相配合，而不是各自默认运行。
